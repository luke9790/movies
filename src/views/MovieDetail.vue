<template>
  <div v-if="movie" class="movie-detail-container">
    ...
    <div class="movie-info">
      ...
      <div v-if="movie.director && movie.director.name">
        ...
        <p
          @click="goToDirectorDetails(movie.director.id)"
  ...
  <div v-else>Caricamento dettagli del film...</div>
</template>
    ...
    const fetchMovie = async () => {
      try {
        ...
        const movieResponse = await fetch(
          `https://api.themoviedb.org/3/movie/${props.id}?api_key=${process.env.VUE_APP_TMDB_API_KEY}`
        );
        if (!movieResponse.ok)
        ...
        const creditsResponse = await fetch(
          `https://api.themoviedb.org/3/movie/${props.id}/credits?api_key=${process.env.VUE_APP_TMDB_API_KEY}`
        );
        if (!creditsResponse.ok)
          throw new Error("Errore nel recupero dei dettagli del cast");
